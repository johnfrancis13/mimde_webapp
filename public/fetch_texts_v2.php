...
$tsql = "WITH GroupedRandom AS (
    ...
    FROM dbo.bias_annotation_data_file
...
$sqla = "UPDATE dbo.bias_annotation_data_file SET live = 1 WHERE id = ? AND qnum = ?";
$sqlb = "UPDATE dbo.bias_annotation_data_file SET locktime = GETDATE() WHERE id = ? AND qnum = ?";
...
foreach ($data as $row) {
    ...
    $stmtA = sqlsrv_query($conn, $sqla, $params);
    ...
    // Update locktime
    $stmtB = sqlsrv_query($conn, $sqlb, $params);
